...
Note that while all subcommands of ~git subtree~ require the ~--prefix~
argument, it is not necessary to set that in the popup.  If the
argument isn't set, then all of the below commands instead read it in
the minibuffer for it.  Other arguments mentioned below are always
read in the minibuffer.
